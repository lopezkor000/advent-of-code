...
def partOne():
	with open('input.txt','r') as file:
		...
		total = 0
		splits = set()
		splits.add(input[0].find('S'))
		for line in input:
			for i,c in enumerate(line):
				if c == "^" and i in splits:
					total += 1
					splits.remove(i)
					splits.add(i-1)
					splits.add(i+1)
		print(f"Splits: {total}")
# partOne()
...
def partTwo():
	with open('input.txt','r') as file:
		...
		for i, line in enumerate(input):
			mapping[i] = list([0]*len(line.strip()))
		
		mapping[0][len(mapping[0])//2] = 1
		
		for row,line in enumerate(input[1:], 1):
			mapping[row] = mapping[row-1].copy()
			for col,c in enumerate(line):
				if c == '^':
					mapping[row][col-1] += mapping[row-1][col]
					mapping[row][col+1] += mapping[row-1][col]
					mapping[row][col] = 0

		print(f"Timelines: {sum(mapping[-1])}")
partTwo()